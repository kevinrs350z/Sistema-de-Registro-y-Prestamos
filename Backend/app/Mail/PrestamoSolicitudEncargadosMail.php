...
class PrestamoSolicitudEncargadosMail extends Mailable
{
    ...
    public function __construct($prestamo)
    {
        ...
        $this->nombreAlumno = trim((string) ($persona?->Nombre . ' ' . $persona?->apellido1 . ' ' . $persona?->apellido2));
        ...
        $this->categorias = $prestamo->equipos
            ? $prestamo->equipos->map(function ($equipo) {
                return $equipo->tipo?->categoria?->nombre;
    ...
    public function build()
    {
        return $this->subject('Nueva solicitud de prestamo')
            ->markdown('emails.prestamos.solicitud-encargados');
    }
}
