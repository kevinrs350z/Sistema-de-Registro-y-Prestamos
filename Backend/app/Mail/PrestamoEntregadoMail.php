...
/**
 * Correo de notificacion cuando un prestamo es entregado fisicamente.
 * Se usa tanto para el alumno como para los encargados.
 */
class PrestamoEntregadoMail extends Mailable
{
    ...
    public function __construct($prestamo, string $nombreAdmin, string $destinatario = 'alumno')
    {
        ...
        $this->fechaEntrega = now()->format('d/m/Y H:i');
    ...
    public function build()
    {
        $subject = $this->destinatario === 'alumno'
            ? 'Entrega confirmada - Prestamo #' . $this->idPrestamo
            : 'Prestamo #' . $this->idPrestamo . ' entregado a ' . $this->nombreAlumno;

        return $this->subject($subject)
            ->markdown('emails.prestamos.entregado');
    }
}
